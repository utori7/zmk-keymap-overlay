...
| `margin` | `48` | 画面端からの余白 |
| `offsetX` / `offsetY` | `0` | `position` で決まる位置からのずれ（px）。オーバーレイをドラッグで動かすと書き込まれる。`position` を選び直すと 0 に戻る |
...
| `setupStep` | なし | 初期設定の案内を次に開くステップ。途中で閉じると入り、「完了」まで見ると消える。入っているあいだ、トレイのメニューは「初期設定を続ける…」と出る。手で消しても構わない |
...
`selectedLayers` は、よく使って覚えたレイヤーでは出さないための見せ方。
L0 を表示していて、表示しないレイヤーに入ったときは消える（L0 を出したままだと、実際に効いているキーと違う配置を見せることになる）。
設定画面の「表示」→「選んだレイヤーのときだけ表示」でチェックを外したレイヤーが `hiddenLayers` に入る。

Ctrl+Alt+数字 やトレイから手で出したレイヤーは、どの見せ方でも表示する。
